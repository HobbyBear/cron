...
func appendingJob(slice *[]int, value int) JobExt {
	var m sync.Mutex
...
func TestChain(t *testing.T) {
	...
	NewChain(append1, append2, append3).Then(append4).Run(ExtContext{})
...
func TestChainRecover(t *testing.T) {
	panickingJob := JobFunc(func(ext ExtContext) {
		log.Println(ext)
...
func (j *countJob) Run(ctx ExtContext) {
	j.m.Lock()
	j.started++
	j.m.Unlock()
	time.Sleep(j.delay)
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("different jobs independent", func(t *testing.T) {
		...
		for i := 0; i < 11; i++ {
			go wrappedJob1.Run(ExtContext{})
			go wrappedJob2.Run(ExtContext{})
		}
