...
func TestFuncPanicRecovery(t *testing.T) {
	...
	cron.Start()
	defer cron.Stop()
	cron.AddFunc("* * * * * ?", func(ctx ExtContext) {
		panic("YOLO")
	})
...
func (d DummyJob) Run(ctx ExtContext) {
	panic("YOLO")
}
...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", func(ctx ExtContext) { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(ctx ExtContext) { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("* * * * * ?", func(ctx ExtContext) { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("@every 2s", func(ctx ExtContext) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", func(ctx ExtContext) {})
	cron.AddFunc("* * * * * ?", func(ctx ExtContext) { wg.Done() })
	id1, _ := cron.AddFunc("* * * * * ?", func(ctx ExtContext) { t.Fatal() })
	id2, _ := cron.AddFunc("* * * * * ?", func(ctx ExtContext) { t.Fatal() })
	cron.AddFunc("0 0 0 31 12 ?", func(ctx ExtContext) {})
	cron.AddFunc("* * * * * ?", func(ctx ExtContext) { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(ctx ExtContext) {})
	cron.AddFunc("* * * * * ?", func(ctx ExtContext) { wg.Done() })
	cron.Schedule(Every(time.Minute), JobFunc(func(ctx ExtContext) {}))
	cron.Schedule(Every(time.Second), JobFunc(func(ctx ExtContext) { wg.Done() }))
	cron.Schedule(Every(time.Hour), JobFunc(func(ctx ExtContext) {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, func(ctx ExtContext) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func (t testJob) Run(ctx ExtContext) {
	t.wg.Done()
}
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), JobFunc(func(ctx ExtContext) {}))
	cron.Schedule(Every(time.Second), JobFunc(func(ctx ExtContext) {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(ctx ExtContext) { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), JobFunc(func(ctx ExtContext) { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", func(ctx ExtContext) {})
		cron.Start()
		cron.AddFunc("* * * * * *", func(ctx ExtContext) { time.Sleep(2 * time.Second) })
		cron.AddFunc("* * * * * *", func(ctx ExtContext) {})
		time.Sleep(time.Second)
